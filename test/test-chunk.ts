...
describe("chunks", () => {
  it("enumerates changed chunks", () => {
    ...
    ist(
      [ch1.fromA, ch1.toA],
      [docA.line(500).from, docA.line(501).from],
      byJSON
    );
    ist(
      [ch1.fromB, ch1.toB],
      [docB.line(500).from, docB.line(501).from],
      byJSON
    );

    ist(
      [ch2.fromA, ch2.toA],
      [docA.line(700).from, docA.line(750).from],
      byJSON
    );
    ist(
      [ch2.fromB, ch2.toB],
      [docB.line(700).from, docB.line(702).from],
      byJSON
    );
    ...
    ist(
      [c2.toA, c2.fromB, c2.toB],
      [docA.line(749).to - ch2.fromA, 13, 15],
      byJSON
    );
  });
  ...
  it("can update chunks for changes", () => {
    let stateA = EditorState.create({ doc: docA }),
      stateB = EditorState.create({ doc: docB });
    ...
    ist(
      [ch2.fromA, ch2.fromB],
      [tr1.newDoc.line(501).from, stateB.doc.line(500).from],
      byJSON
    );
    stateA = tr1.state;

    let tr2 = stateB.update({
      changes: [
    ...
    ist(
      [ch3.fromA, ch3.toA],
      [stateA.doc.line(601).from, stateA.doc.line(602).from],
      byJSON
    );
    ist(
      [ch3.fromB, ch3.toB],
      [tr2.newDoc.line(600).from, tr2.newDoc.line(601).from],
      byJSON
    );
    ist(
      [ch5.fromA, ch5.toA],
      [stateA.doc.length - 9, stateA.doc.length + 1],
      byJSON
    );
    ist(
      [ch5.fromB, ch5.toB],
      [tr2.newDoc.length - 13, tr2.newDoc.length + 1],
      byJSON
    );
  });
  ...
  it("can handle deleting updates", () => {
    ...
    ist(chunks1.length, 3);
    ist(
      chunks1.map((c) => c.fromA),
      [0, 4283, 6083],
      byJSON
    );
    ist(
      chunks1.map((c) => c.fromB),
      [0, 4383, 6181],
      byJSON
    );
  });
  ...
  it("works for changes inside changed code", () => {
    ...
    ist(
      updated
        .map((ch) => `${ch.fromA}-${ch.toA}/${ch.fromB}-${ch.toB}`)
        .join(" "),
      "2-7/2-2"
    );
  });
  ...
  it("tracks chunk precision", () => {
    ...
    let tr = sB.update({
      changes: { from: sB.doc.length, insert: "b\n".repeat(999) },
    });
    let updated = Chunk.updateB(chs, sA.doc, tr.newDoc, tr.changes, {
      scanLimit: 100,
    });
